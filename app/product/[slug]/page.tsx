...
const productDirectory = path.join(process.cwd(), 'content/product');

function getAllProductSlugs(dir: string): string[] {
  const slugs: string[] = [];
...
export function generateStaticParams() {
  const slugs = getAllProductSlugs(productDirectory);
  return slugs.map((slug) => ({ slug }));
}

export default function ProductDetailPage({ params }: { params: { slug: string } }) {
  const fullPath = path.join(productDirectory, `${params.slug}.md`);
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="flex-grow pt-28 pb-16 px-4 sm:px-6 lg:px-8 max-w-4xl mx-auto w-full">
        ...
        <Link href="/product" className="text-blue-600 hover:text-blue-700 flex items-center gap-2 mb-8">
